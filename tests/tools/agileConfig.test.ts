...
describe('manage_agile_config', () => {
  ...
  beforeEach(() => {
    ...
    handler = (mockServer.registerTool as any).mock.calls.find((c: any) => c[0] === 'manage_agile_config')[2];
  ...
  it('should get the current agile methodology config', async () => {
    ...
    expect(result.content[0].text).toEqual(JSON.stringify({ type: 'scrum', settings: { sprintLength: 2 } }, null, 2));
  });
  ...
  it('should set a new agile methodology config', async () => {
    ...
    const result = await handler({ action: 'set', type: newConfig.type, settings: newConfig.settings });
    ...
    expect(result.content[0].text).toContain('Agile methodology set to kanban with settings: {"wipLimit":5}');
  });
