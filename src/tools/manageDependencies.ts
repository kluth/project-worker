...
export function registerManageDependencies(server: McpServer) {
  server.registerTool(
    'manage_dependencies',
    {
    ...
    async ({ taskId, blockerId, action }) => {
      ...
      if (!blocker) return { isError: true, content: [{ type: 'text', text: `Blocker ${blockerId} not found` }] };

      const oldBlockedBy = [...(task.blockedBy || [])];
      
      if (action === 'add') {
        if (task.blockedBy?.includes(blockerId)) {
          return { content: [{ type: 'text', text: `Task ${taskId} is already blocked by ${blockerId}` }] };
        }
        ...
        if (!task.blockedBy?.includes(blockerId)) {
          return { content: [{ type: 'text', text: `Task ${taskId} is not blocked by ${blockerId}` }] };
        }
