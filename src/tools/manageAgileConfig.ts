...
import { z } from 'zod';
import { configManager, AgileMethodologyConfig } from '../config.js';
  ...
  server.registerTool(
    'manage_agile_config',
    {
      ...
      inputSchema: z.object({
        ...
        type: z.enum(['scrum', 'kanban', 'waterfall', 'lean', 'prince2', 'custom']).optional().describe('The type of agile methodology'),
        settings: z.record(z.any()).optional().describe('Methodology-specific settings (e.g., { sprintLength: 2 })'),
      }).shape,
    },
    async ({ action, type, settings }) => {
      ...
      if (action === 'set') {
        if (!type) {
          return { isError: true, content: [{ type: 'text', text: 'Methodology type is required for "set" action.' }] };
        }
        ...
        return {
          content: [{ type: 'text', text: `Agile methodology set to ${type} with settings: ${JSON.stringify(settings || {})}` }]
        };
      }
