...
export function registerManageDiscussions(server: McpServer) {
  server.registerTool(
    'manage_discussions',
    {
    ...
    async ({ action, discussionId, title, content, author, tags }) => {
      ...
      if (action === 'list') {
        ...
        // Return summary
        const summary = all.map(d => ({ id: d.id, title: d.title, status: d.status, msgCount: d.messages.length }));
        return { content: [{ type: 'text', text: JSON.stringify(summary, null, 2) }] };
      }

      if (action === 'start') {
        if (!title || !content) return { isError: true, content: [{ type: 'text', text: 'Title and content required to start' }] };
      ...
      if (!discussionId) return { isError: true, content: [{ type: 'text', text: 'discussionId required' }] };
      const discussion = await db.getDiscussionById(discussionId);
      if (!discussion) return { isError: true, content: [{ type: 'text', text: `Discussion ${discussionId} not found` }] };
      ...
      if (action === 'reply') {
        if (!content) return { isError: true, content: [{ type: 'text', text: 'Content required for reply' }] };
